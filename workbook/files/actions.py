...
WORKBOOK_DIR = os.path.dirname(BASE_DIR)
ROOT_DIR = os.path.dirname(WORKBOOK_DIR)

pbars = []
# ['0x11', '그리디', 'https://www.acmicpc.net/workbook/view/7320']
def parse_links():
  attrs = []
  with open(os.path.join(BASE_DIR, 'links.txt'), encoding="UTF-8") as f:
    for line in f:
...
def parse_category():
  category = []
  with open(os.path.join(BASE_DIR, 'problems.txt'), encoding="UTF-8") as f:
    for line in f:
...
'''
  ...
  for attr in attrs:
    if len(attr) < 3: # No workbook
      ...
      continue
    solution_num = 0
    solution_path = os.path.join(WORKBOOK_DIR, attr[0])
    ...
    for prob_id, prob_name in problem_infos:
      ...
      if codes[:100] == txt[:100]:
        ...
        solution_num += 1
        relative_file_path = os.path.relpath(file_path, BASE_DIR).replace(os.sep, '/')
        code_attr = f'[정답 코드]({relative_file_path.replace(" ", "%20")}.cpp)'
        MAX_DIFFERENT_SOLUTION = 9
        for i in range(1, MAX_DIFFERENT_SOLUTION+1):
          if os.path.exists(file_path+'_'+str(i)+'.cpp'):
            extra_file_path = os.path.relpath(file_path+'_'+str(i), BASE_DIR).replace(os.sep, '/')
            code_attr += f", [별해 {i}]({extra_file_path}.cpp)"
        prob_table += f'| {CATEGORY[category_idx]} | {prob_id} | [{prob_name}](https://www.acmicpc.net/problem/{prob_id}) | {code_attr} |\n'
    chapter_md_path = os.path.join(BASE_DIR, attr[0] + '.md')
...
def gen_total_workbook(attrs):
  with open(os.path.join(ROOT_DIR, 'README.md'), 'w', encoding="UTF-8") as f:
    f.write('''본인이 만든 것이 아닌, [바킹독](https://github.com/encrypted-def/basic-algo-lecture)의 자료입니다.
...
| :--: | :--: | :--: |\n''')
    for attr, pbar in zip(attrs, pbars):
      if len(attr) < 3: # No workbook
        ...
        f.write(f'| {attr[0]} | [{attr[1]}](workbook/files/{attr[0].replace(" ", "%20")}.md) | {pbar} |\n')
